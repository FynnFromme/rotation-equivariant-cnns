...
class TestEquivariance(unittest.TestCase):
    """Checks 90° rotational equivariance as well as the output shape of every SE(2,N)-CNN layer."""
    
    def test_LiftConv(self):
        # Test 1: without bias
        in_shape = (4, 4, 1)
        model = get_model(in_shape, se2n_cnn.layers.LiftConv(channels=1, orientations=4, ksize=3, padding='VALID', use_bias=False))
        ...
        # Test 2: with bias
        in_shape = (4, 4, 1)
        model = get_model(in_shape, se2n_cnn.layers.LiftConv(channels=1, orientations=4, ksize=3, padding='VALID', use_bias=True))
        
        self.assertTrue(check_equivariance(model, in_shape))
        self.assertTrue(check_shape(model, in_shape, (2, 2, 4, 1)))
        
    def test_SE2NConv(self):
        # Test 1: without bias
        in_shape = (4, 4, 4, 1)
        model = get_model(in_shape, se2n_cnn.layers.SE2NConv(channels=1, ksize=3, padding='VALID', use_bias=False))
    ...
    def test_BatchNormalization(self):
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.BatchNormalization())
        
        self.assertTrue(check_equivariance(model, in_shape))
        self.assertTrue(check_shape(model, in_shape, in_shape))
        
    def test_RotationPooling(self):
        # Test 1: with spatial dimensions and keepdims=True
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.RotationPooling(tf.reduce_max, keepdims=True))
        ...
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.RotationPooling(tf.reduce_max, keepdims=False))
    ...
    def test_SpatialPooling(self):
        ...
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.SpatialPooling(ksize=(2,2), strides=(2,2), padding='VALID'))
    ...
    def test_ReduceSpatial(self):
        ...
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.ReduceSpatial(tf.reduce_max, keepdims=True))
        ...
        in_shape = (4,4,4,1)
        model = get_model(in_shape, se2n_cnn.layers.ReduceSpatial(tf.reduce_max, keepdims=False))
