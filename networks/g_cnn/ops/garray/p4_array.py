...
class P4Array(MatrixGArray):
    """
    A P4 Array is a Array that stores elements of the p4 group.
    ...
    A homogeneous matrix representation of this group is given by T(u, v) R(r), where
    T = [[ 1, 0, u],
         [ 0, 1, v],
         [ 0, 0, 1]]
    ...
    def __init__(self, data: np.ndarray, p: str = 'int'):
        """Initializes a p4 GArray with the data provided.
        ...
        Raises:
            AssertionError: If the data is not integer.
        """
        data = np.asarray(data)
        assert data.dtype == int
        
        # supports left actions on P4 and Z2
    ...
    def hmat2int(self, mat_data: np.ndarray) -> np.ndarray:
        ...
        u = mat_data[..., 0, 2]
        v = mat_data[..., 1, 2]
        r = ((np.arctan2(s, c) / np.pi * 2) % 4).astype(int) # arctan2 calculates the angle given cos and sin
...
def identity(shape: tuple = (), p: str = 'int') -> P4Array:
    """Returns a P4Array of P4 identity elements of a given shape.

    Args:
        shape (tuple, optional): The shape of the array. Defaults to ().
        p (str, optional): The parameterization of the group elements. Defaults to 'int'.

    Returns:
        P4Array: The P4Array in the given parameterization.
    """
    e = P4Array(np.zeros(shape + (3,), dtype=int), 'int')
    return e.reparameterize(p)


def rand(minu, maxu, minv, maxv, shape=(), p: str = 'int') -> P4Array:
    """Returns a P4Array of random P4 group elements of a given shape.

    Args:
        minu (int): The minimum u translation.
        maxu (int): The maximum u translation.
        minv (int): The minimum v translation.
        maxv (int): The maximum v translation.
        shape (tuple, optional): The shape of the array. Defaults to ().
        p (str, optional): The parameterization of the group elements. Defaults to 'int'.
    ...
    Returns:
        P4Array: The P4Array in the given parameterization.
    """
    data = np.zeros(shape + (3,), dtype=np.int64)
    data[..., 0] = np.random.randint(0, 4, shape)
    data[..., 1] = np.random.randint(minu, maxu, shape)
    data[..., 2] = np.random.randint(minv, maxv, shape)
    return P4Array(data=data, p='int').reparameterize(p)
...
def r_range(start: int = 0, stop: int = 4, step: int = 1) -> P4Array:
    """Initializes a P4Array consisting of a range of rotations.

    Args:
        start (int, optional): The included start rotation. Defaults to 0.
        stop (int, optional): The excluded stop rotation. Defaults to 4.
        step (int, optional): Spacing between the rotations. Defaults to 1.

    Returns:
        P4Array: P4Array consisting of a range of rotations.
    """
    assert stop > 0
    assert stop <= 4
    assert start >= 0
...
def u_range(start: int = -1, stop: int = 2, step: int = 1) -> P4Array:
    """Initializes a P4Array consisting of a range of u translations.

    Args:
        start (int, optional): The included start translation. Defaults to -1.
        stop (int, optional): The excluded stop translation. Defaults to 2.
        step (int, optional): Spacing between the rotations. Defaults to 1.

    Returns:
        P4Array: P4Array consisting of a range of u translations.
    """
    m = np.zeros((stop - start, 3), dtype=int)
...
def v_range(start=-1, stop=2, step=1):
    """Initializes a P4Array consisting of a range of v translations.
    ...
    Returns:
        P4Array: P4Array consisting of a range of v translations.
    """
    m = np.zeros((stop - start, 3), dtype=int)
...
def meshgrid(r: P4Array = r_range(), u: P4Array = u_range(), v: P4Array = v_range()) -> P4Array:
    """Combines the different ranges to a P4Array containing group elements for every combination of values from these ranges.

    Args:
        r (P4Array, optional): The range of rotations. Defaults to r_range(0, 4).
        u (P4Array, optional): The range of u translations. Defaults to u_range(-1, 2).
        v (P4Array, optional): The range of v translations. Defaults to v_range(-1, 2).

    Returns:
        P4Array: P4Array containing group elements for every combination of values from these ranges.
    """
